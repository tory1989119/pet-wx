...
function table(data,pageNum){
	...
	for (var i = 0; i < data.content.length; i++) { 
		...
		str = str + '<td>' + data.content[i].acreage + '</td>';
		str = str + '<td>' + getWareHouseTypeName(data.content[i].type) +'</td>';
		...
		str = str + '<td><a href="javascript:void(0)" onclick="getWareHouseInfo(\'' + data.content[i].id + '\')">仓储详情</a></td>';
		str = str + '</tr>';
    }
	$("#tbodyId").html(str);
}
...
function getWareHouseInfo(id){
	...
	var index = layer.open({
	    ...
	    fix: false, //不固定
	    maxmin: true,
	    content: 'wareHouseInfoPage.htm?id=' + id
	});
	layer.full(index);
}
...
function queryWareHouseRecord(){
	...
	var index = layer.open({
	    ...
	    fix: false, //不固定
	    maxmin: true,
	    content: 'wareHouseRecordPage.htm'
	});
	layer.full(index);
}