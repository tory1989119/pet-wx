...
function search(pageNum){
	...
	layer.load(2);//遮罩层
	$.ajax({
	      url: "queryTruckOwnerExamine.htm",
	      datatype: 'json',
	      type: "post",
	      data: {
...
function table(data,pageNum){
	...
	for (var i = 0; i < data.content.length; i++) { 
		...
		str = str + '<td><a href="javascript:void(0)" onclick="getDriverInfo(\'' + data.content[i].id + '\')">司机详情</a> </td>';
		str = str + '</tr>';
    }
	$("#tbodyId").html(str);
}
...
function getDriverInfo(id){
	...
	var index = layer.open({
	    ...
	    fix: false, //不固定
	    maxmin: true,
	    content: 'driverExamineInfoPage.htm?id=' + id
	});
	layer.full(index);
}
