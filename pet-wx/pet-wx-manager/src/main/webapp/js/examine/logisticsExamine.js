...
function table(data,pageNum){
	if(data.content == null || data.content.length <= 0){
		$("#tbodyId").html('<tr ><td colspan="12">无数据</td></tr>');
	...
	for (var i = 0; i < data.content.length; i++) { 
		...
		if(data.content[i].companyRepresents != null){
			str = str + '<td>' + data.content[i].companyRepresents + '</td>';
		}else{
			str = str + '<td></td>';
		}
		...
		str = str + '<td><a href="javascript:void(0)" onclick="getLogisticsInfo(\'' + data.content[i].id + '\')">企业详情</a> </td>';
		str = str + '</tr>';
    }
	$("#tbodyId").html(str);
}
...
function getLogisticsInfo(id){
	...
	var index = layer.open({
	    ...
	    fix: false, //不固定
	    maxmin: true,
	    content: 'logisticsExamineInfoPage.htm?id=' + id
	});
	layer.full(index);
}
